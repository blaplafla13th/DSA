...
public class Card implements Comparable<Card> {
    ...
    public int compareTo(Card card) {
        ...
        if (Arrays.asList(RANKS).indexOf(this.rank) > Arrays.asList(RANKS).indexOf(card.rank))
            return 1;
        else if (Arrays.asList(RANKS).indexOf(this.rank) < Arrays.asList(RANKS).indexOf(card.rank))
            return -1;
        else return Integer.compare(Arrays.asList(SUITS).indexOf(this.suit), Arrays.asList(SUITS).indexOf(card.suit));
    }
